...
namespace lab
{
	public partial class Lab11 : Form
	{
		...
		private void buttonAddTestFiles_Click(object sender, EventArgs e)
		{
			// Создаём коллекцию объектов, подписанных на интерфейс IFileSystemNode
			List<IFileSystemNode> fileSystemNodes = new List<IFileSystemNode>
			{
				new BrowserFile("node_document.txt", Path.Join(currentDirectory.Path, "document.txt")),
				new BrowserDirectory("node_folder",  Path.Join(currentDirectory.Path, "node_folder"))
			};

			Debug.WriteLine(Path.Join(currentDirectory.Path, "node_folder"));

			// Обрабатываем все объекты единым способом через интерфейс
			foreach (var node in fileSystemNodes)
			{
				node.Create(); // Создаём объект
				Debug.WriteLine(node); // Выводим информацию об объекте
				node.Rename(node.Name + "_Renamed"); // Переименовываем объект
				ScanWorkDirForEntries();
				ClearDataBindings();
				SetupDataBindings();
			}
